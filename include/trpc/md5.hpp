...
static constexpr IntermediateData kInitialIntermediateData{0x67452301, 0xefcdab89, 0x98badcfe,
                                                           0x10325476};
//////////////////////////////////////////////////////////////////////////////
...
struct MD5CE {
    ...
    static uint8_t GetPaddedMessageByte(const char *data,
                                        const uint32_t n,
                                        const uint32_t m,
                                        const uint32_t i) {
        //    DCHECK_LT(i, m);
    ...
    static uint32_t GetPaddedMessageWord(const char *data,
                                         const uint32_t n,
                                         const uint32_t m,
                                         const uint32_t i) {
        //    DCHECK_EQ(i % 4, 0u);
    ...
    static RoundData GetRoundData(const char *data,
                                  const uint32_t n,
                                  const uint32_t m,
                                  const uint32_t i) {
        //    DCHECK_EQ(i % 64, 0u);
        ...
        //    DCHECK_EQ(m % 64, 0u);
        return RoundData{
            {GetPaddedMessageWord(data, n, m, i), GetPaddedMessageWord(data, n, m, i + 4),
    ...
    static IntermediateData ApplyStep(const uint32_t i,
    ...
    static IntermediateData ProcessMessage(const char *message, const uint32_t n) {
        ...
        for (uint32_t offset = 0; offset < m; offset += 64) {
            ...
            for (uint32_t i = 0; i < 64; ++i) intermediate1 = ApplyStep(i, data, intermediate1);
